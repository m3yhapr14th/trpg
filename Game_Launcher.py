...
print("{0}이 길을 가다가 {1}를 마주쳤다.".format(user.name, Hydra_Lv1.name))
sleep(1)
print("[{0} 출몰]\n[체력: {1}, 공격력: {2}, 방어력: {3}]".format(Hydra_Lv1.name, Hydra_Lv1.hp, Hydra_Lv1.power, Hydra_Lv1.defense))
sleep(0.5)
# 전투
answer = input("전투를 시작하겠습니까?\n1. 전투 시작\n2. 도망\n선택: ")
if answer == '1':
    while Hydra_Lv1.hp > 0:
        battle = input("1. 공격\n2. 도망가기\n선택: ")
        if battle == '1':
            user.attack(Hydra_Lv1.name)
            sleep(0.2)
            Hydra_Lv1.damaged(user.power)
            sleep(1)
            Hydra_Lv1.attack(user.name)
            sleep(0.2)
            user.damaged(Hydra_Lv1.power)
            sleep(1)
            # if Hydra_Lv1.hp <= 0:
